...
public class ServletCategorias extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		if (cmd.length() == 1 || cmd.length() == 2 || cmd.length() == 3) {
			isNotIndex = false;
			RequestDispatcher rd = null;
			Categoria categoria2 = null;
			List<Produto> produtos = new ArrayList<Produto>();
			try {
				categoria2 = new CategoriaDAO().procurarCategoria(Integer
						.valueOf(cmd));
			...
			try {
				...
				produtos = new ProdutoDAO()
						.procurarProdutoPorCategoria(categoria2.getCategoria());
			...
			if (cmd != null) {
				...
				try {
					...
					categoria
							.setCategoria(request.getParameter("txtCategoria"));
					categoria.setLinha(request.getParameter("txtLinha"));
					categoria.setFaixaEtaria(request
							.getParameter("txtFaixaEtaria"));
			...
			try {
				...
				// incluir aluno
					...
					rd = request
							.getRequestDispatcher("/formExcluirCategoria.jsp");
					...
					session.setAttribute("categoria", categoria);
					rd = request
							.getRequestDispatcher("/formAtualizarCategoria.jsp");
